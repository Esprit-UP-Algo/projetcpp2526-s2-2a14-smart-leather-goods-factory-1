...
products::products(int idEmploye, QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::products)
...
void products::loadProducts()
{
    ...
    QList<QStringList> sampleData = {
        {"P001", "Portefeuille cuir", "Accessoires", "15/06/2024", "22/06/2024", "actif", "45.50"},
        {"P002", "Sac à main", "Maroquinerie", "16/06/2024", "23/06/2024", "En attente", "89.90"},
        {"P003", "Ceinture", "Accessoires", "17/06/2024", "24/06/2024", "actif", "29.90"},
        {"P004", "Veste cuir", "Vêtements", "18/06/2024", "25/06/2024", "arrété", "199.00"},
        {"P005", "Chaussures", "Chaussures", "19/06/2024", "26/06/2024", "actif", "79.50"},
        {"P006", "Sac de voyage", "Maroquinerie", "20/06/2024", "27/06/2024", "En attente", "149.00"}
    };

    for (const QStringList &rowData : sampleData) {
        QDateTime orderDate = QDateTime::fromString(rowData[3], "dd/MM/yyyy");
        QDateTime deliveryDate = QDateTime::fromString(rowData[4], "dd/MM/yyyy");

        addProductToTable(rowData[0], rowData[1], rowData[2],
                          orderDate, deliveryDate, rowData[5], rowData[6]);
    }
}

void products::addProductToTable(const QString &id, const QString &name,
                                 const QString &category, const QDateTime &orderDate,
                                 const QDateTime &deliveryDate, const QString &status,
                                 const QString &price)
{
    ...
    ui->tableWidget->setItem(row, 3, new QTableWidgetItem(orderDate.toString("dd/MM/yyyy")));
    ui->tableWidget->setItem(row, 4, new QTableWidgetItem(deliveryDate.toString("dd/MM/yyyy")));
    ui->tableWidget->setItem(row, 5, new QTableWidgetItem(status));
    ui->tableWidget->setItem(row, 6, new QTableWidgetItem(price + " €"));
                                    ...
                                    const QString &price)
{
    if (row >= 0 && row < ui->tableWidget->rowCount()) {
        ...
        ui->tableWidget->item(row, 3)->setText(orderDate.toString("dd/MM/yyyy"));
        ui->tableWidget->item(row, 4)->setText(deliveryDate.toString("dd/MM/yyyy"));
        ui->tableWidget->item(row, 5)->setText(status);
        ui->tableWidget->item(row, 6)->setText(price + " €");
...
{
    hide();
    pagemachine *pdedd = new pagemachine(m_idEmploye, this);
    pdedd->show();
}
...
void products::on_pushButton_clicked()
{
    ...
    QDateTimeEdit *dateOrderEdit = new QDateTimeEdit(QDateTime::currentDateTime());
    dateOrderEdit->setDisplayFormat("dd/MM/yyyy");
    dateOrderEdit->setCalendarPopup(true);

    QDateTimeEdit *dateDeliveryEdit = new QDateTimeEdit(QDateTime::currentDateTime().addDays(7));
    dateDeliveryEdit->setDisplayFormat("dd/MM/yyyy");
    dateDeliveryEdit->setCalendarPopup(true);
    ...
    mainLayout->addWidget(dateOrderEdit);
    mainLayout->addWidget(new QLabel("📅 Date livraison :"));
    mainLayout->addWidget(dateDeliveryEdit);
    ...
    connect(btnSave, &QPushButton::clicked, [ajoutDialog, idEdit, nameEdit, categoryEdit,
                                             dateOrderEdit, dateDeliveryEdit, statusCombo,
                                             priceEdit, this]() {
        if (idEdit->text().isEmpty() || nameEdit->text().isEmpty() ||
            categoryEdit->text().isEmpty() || priceEdit->text().isEmpty()) {
        ...
        addProductToTable(idEdit->text(),
                          nameEdit->text(),
                          categoryEdit->text(),
                          dateOrderEdit->dateTime(),
                          dateDeliveryEdit->dateTime(),
...
void products::on_pushButton_2_clicked()
{
    ...
    QDateTime orderDate = QDateTime::fromString(ui->tableWidget->item(currentRow, 3)->text(), "dd/MM/yyyy");
    QDateTime deliveryDate = QDateTime::fromString(ui->tableWidget->item(currentRow, 4)->text(), "dd/MM/yyyy");
    QString status = ui->tableWidget->item(currentRow, 5)->text();
    QString price = ui->tableWidget->item(currentRow, 6)->text().replace(" €", "");
    ...
    QDateTimeEdit *dateOrderEdit = new QDateTimeEdit(orderDate);
    dateOrderEdit->setDisplayFormat("dd/MM/yyyy");
    dateOrderEdit->setCalendarPopup(true);

    QDateTimeEdit *dateDeliveryEdit = new QDateTimeEdit(deliveryDate);
    dateDeliveryEdit->setDisplayFormat("dd/MM/yyyy");
    dateDeliveryEdit->setCalendarPopup(true);
    ...
    connect(btnSave, &QPushButton::clicked, [modifierDialog, currentRow, nameEdit, categoryEdit,
                                             dateOrderEdit, dateDeliveryEdit, statusCombo,
                                             priceEdit, this]() {
        if (nameEdit->text().isEmpty() || categoryEdit->text().isEmpty() || priceEdit->text().isEmpty()) {
        ...
        updateProductInTable(currentRow,
                             nameEdit->text(),
                             categoryEdit->text(),
                             dateOrderEdit->dateTime(),
                             dateDeliveryEdit->dateTime(),
