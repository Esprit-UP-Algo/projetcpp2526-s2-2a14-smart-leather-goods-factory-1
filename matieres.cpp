...
void Matieres::loadMatieres()
{
    ...
    if (query.exec()) {
        while (query.next()) {
            addMatiereToTable(query.value(0).toString(),
...
void Matieres::on_pushButton_clicked()
{
    ...
    connect(btnSave, &QPushButton::clicked, [ajoutDialog, idEdit, typeEdit, couleurEdit,
        ...
        QSqlQuery query;
        query.prepare("INSERT INTO MATIERE (ID_MATIERE, TYPE_MATIERE, COULEUR, QUALITE, PRIX, STOCK, FOURNISSEUR) "
                      "VALUES (:id, :type, :couleur, :qualite, :prix, :stock, :fournisseur)");
        ...
        if (query.exec()) {
            ...
            QMessageBox::information(ajoutDialog, "Succès",
                                   "✅ La matière a été ajoutée avec succès dans la base de données!");
            ajoutDialog->accept();
        } else {
            QMessageBox::critical(ajoutDialog, "Erreur",
...
void Matieres::on_pushButton_2_clicked()
{
    ...
    QLineEdit *fournisseurEdit = new QLineEdit();
    fournisseurEdit->setText(fournisseur);
    fournisseurEdit->setAlignment(Qt::AlignCenter);
    ...
    connect(btnSave, &QPushButton::clicked, [modifierDialog, currentRow, idEdit, typeEdit, couleurEdit,
                                              qualiteCombo, prixEdit, stockEdit, fournisseurEdit, this]() {
        ...
        QSqlQuery query;
        query.prepare("UPDATE MATIERE SET TYPE_MATIERE=:type, COULEUR=:couleur, QUALITE=:qualite, PRIX=:prix, STOCK=:stock, FOURNISSEUR=:fournisseur WHERE ID_MATIERE=:id");
        query.bindValue(":type", typeEdit->text());
        query.bindValue(":couleur", couleurEdit->text());
        query.bindValue(":qualite", qualiteCombo->currentText());
        query.bindValue(":prix", prix);
        ...
        if (query.exec()) {
            ...
            QMessageBox::information(modifierDialog, "Succès",
                                   "✅ La matière a été mise à jour avec succès dans la base de données!");
            modifierDialog->accept();
        } else {
            QMessageBox::critical(modifierDialog, "Erreur",
...
void Matieres::on_pushButton_3_clicked()
{
    ...
    if (reply == QMessageBox::Yes) {
        ...
        if (query.exec()) {
            ...
            QMessageBox::information(this, "Succès", "✅ Matière supprimée de la base de données avec succès!");
            qDebug() << "Matière supprimée de la BDD:" << id << "- Type:" << type;
        } else {
