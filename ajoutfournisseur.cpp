...
void AjoutFournisseur::setAdresseComplete(const QString &adresse)
{
    QStringList parts = adresse.split(" - ");
    if (parts.size() >= 5) {
        paysEdit->setText(parts[0]);
        regionEdit->setText(parts[1]);
        villeEdit->setText(parts[2]);
        rueEdit->setText(parts[3]);
        localisationExacteEdit->setText(parts[4]);
    } else {
        // Fallback for old simple addresses
        paysEdit->setText(adresse);
    }
}
