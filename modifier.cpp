...
Modifier::Modifier(QWidget *parent) : QDialog(parent)
{
    ...
    mainLayout->addWidget(new QLabel("👔 Employé responsable :"));
    mainLayout->addWidget(employeCombo);
                             ...
                             const QString &etat, const QString &montant, const QString &modePaiement, const QString &employeId)
{
    ...
    int empIndex = employeCombo->findData(employeId);
    if (empIndex >= 0) {
        employeCombo->setCurrentIndex(empIndex);
    }
}
